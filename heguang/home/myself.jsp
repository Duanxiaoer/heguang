                        ...
                        <table style="width: 150%;overflow: scroll">
                            ...
                            <%
                                ...
                                <tr style="background-color: <%=color[index]%>">
                                    ...
                                    <%  String status="等待咨询师接受";
                                        if(resultSetZX.getString("cancel").equals("1")){
                                            status="咨询师已接受";
                                        }
                                        else if (resultSetZX.getString("cancel").equals("2")){
                                            status="咨询师已拒绝（已退款）";
                                        }
                                    %>
                                    <td id="status<%=resultSetZX.getString("id")%>"><%=status%></td>
                                    <%
                                        if (session.getAttribute("zxsName") != null&&resultSetZX.getString("cancel").equals("0")){
                                    %>
                                    <div>
...
<script type="text/javascript">
    function receive(tablename,cancel,id) {
        var flag=confirm("确定操作？接受或拒绝后将无法更改！");
        if(flag==true){
            $.ajax({
                url:"updateMyself.jsp",
                type:"POST",
                data:{"tableName":tablename,"id":id,"cancel":cancel},
                success:function(){
                    if(cancel==1){
                        alert("接受成功！");
                        document.getElementById("status"+id).innerText="已接受";
                    }
                    else{
                    ...
                    document.getElementById("jj1"+id).innerHTML="";
                    document.getElementById("jj2"+id).innerHTML="";
                }
