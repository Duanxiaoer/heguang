        ...
        <div class="row">
            ...
            <div class="col-lg-4 col-md-4 col-sm-12 col-12">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">修改咨询师权限</li>
                </ol>
            </div>
        </div>
        <!-- end row-->
                            ...
                            <%
                                ...
                                try {
                                    if (!keywords.equals("")) {
                                        ...
                                        rs = db.getResultSetOrdered(rs,"zxs","name" ,keywords);
                                    } else {
                                        rs = db.queryZxs();
                                    }
//                                  循环获取数据
                            ...
                            <tr>
                                ...
                                <%
                                    if(rs.getString("bjzl").equals("1")){%>

                                <td>可编辑<input type="radio" id="bjzl<%=rs.getString("id")%>" name="bjzl<%=rs.getString("id")%>" checked onclick="changebjzl('zxs',1,'<%=rs.getString("id")%>')">
                                不可编辑<input type="radio" id="bjzl<%=rs.getString("id")%>" name="bjzl<%=rs.getString("id")%>" onclick="changebjzl('zxs',0,'<%=rs.getString("id")%>')"></td>
                                <%}else{%>
                                <td>可编辑<input type="radio" id="bjzl<%=rs.getString("id")%>" name="bjzl<%=rs.getString("id")%>" onclick="changebjzl('zxs',1,'<%=rs.getString("id")%>')">
                                不可编辑<input type="radio" id="bjzl<%=rs.getString("id")%>" name="bjzl<%=rs.getString("id")%>" checked onclick="changebjzl('zxs',0,'<%=rs.getString("id")%>')"></td>
                                <%}%>
                            </tr>
                            <%
                                    }
...
<script>
    ...
    function changebjzl(tableName, bjzl ,id) {
        $.ajax({
            url: "ggbjzl.jsp",
            type: "POST",
            data: {"tableName": tableName,"bjzl":bjzl, "id": id},
            success: function () {
                alert("修改咨询师权限成功!");
            }
